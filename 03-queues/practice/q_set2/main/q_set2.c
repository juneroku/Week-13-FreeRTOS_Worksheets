...
void app_main(void) {
    ESP_LOGI(TAG, "Queue Sets Implementation Lab Starting... (Experiment 2: Disable Sensor)");
    ...
    if (xSensorQueue && xUserQueue && xNetworkQueue &&
        ...
        // ===== Producers / Sources =====
        // ★ ทดลองที่ 2: ปิดใช้งานแหล่งข้อมูล Sensor (คอมเมนต์บรรทัดนี้)
        ...
        // LED startup sequence (optional visual check)
        for (int i = 0; i < 3; i++) {
            gpio_set_level(LED_SENSOR, 1);
            vTaskDelay(pdMS_TO_TICKS(100));
            gpio_set_level(LED_SENSOR, 0);
